...
open IN, "/DBS/DB_temp/zhangLab/annovar/humandb_b37/hg19_cosmic68.txt" or die "ERROR: open $pesoap: $!";
if($vcf=~/gz$/) {
...
while(defined($line=<IN>)) {
	...
	if((length($colu[3])>1 || length($colu[4])>1) || ($colu[3] eq "-" || $colu[4] eq "-") ){
