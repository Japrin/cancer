...
=head1 Usage
    ...
    -l  pep length [default: 11]
    -m  max output sequence length [default 5000]
...
my ($opt_h,$opt_e,$opt_o,$opt_l,$opt_m,$opt_s);
GetOptions("h"	=>\$opt_h,"e=s"=>\$opt_e,"o=s"=>\$opt_o,"l=i"=>\$opt_l,"m=i"=>\$opt_m,"s=s"=>\$opt_s);
...
if(!defined($opt_l)) { $opt_l=11; }
if(!defined($opt_m)) { $opt_m=5000; }
...
open $out4,">","$opt_o.used.maf" or die "$!";
...
while(<$in>)
{
    ...
    my @F=split /\t/;
    my ($chr,$pos,$hugo_symbol,$enst_id,$aaChange,$varClass,$sampleID,$ref,$alt)=@F[4,5,0,39,47,8,15,35,36];
