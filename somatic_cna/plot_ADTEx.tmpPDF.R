...
if(length(args)<4)
{
	cat("usage: plot_ADTEx.R <sampleID> <cnv.result> <cnv.result.segment> <outDir> [zygosity.res] [zygosity.res.segment]\n")
	q()
}
...
for (i in c(1:22,'X','Y')) 
{
	if(length(args)>=6)
	{
		...
		pdf(paste(outDir,"/",sampleID,".chr",i,".pdf",sep=""),width=14,height=15, pointsize = 20, bg = "white")
	...
	}else
	{
		...
		pdf(paste(outDir,"/",sampleID,".chr",i,".pdf",sep=""),width=14,height=6, pointsize = 20, bg = "white")
	...
	if(length(tt)>0)
	{
		...
		legend(x="topright",legend=paste("CN=",as.numeric(names(table(binTable$copy.number))),sep=""),col=as.integer(names(table(binTable$copy.number)))+1, pch=20, xpd=NA, inset=c(-0.2,0.00),cex=2)
	}
	## LOH plot
	if(length(args)>=6)
	{
		...
		legend(x="topright",legend=c("LOH","HET","ASCNA"),col=c(2:4), pch=20, xpd=NA, inset=c(-0.22,0.00),cex=2)
...
if(length(args)>=6)
{
	...
	pdf(paste(outDir,"/",sampleID,".whole.pdf",sep=""),width=25,height=9)
...
}else
{
	...
	pdf(paste(outDir,"/",sampleID,".whole.pdf",sep=""),width=25,height=5)
	op <- par(mar=c(2,6,6,11)+0.1)
...
legend(x="topright",legend=paste("CN=",as.numeric(names(table(binTable$copy.number))),sep=""),col=as.integer(names(table(binTable$copy.number)))+1, pch=20, xpd=NA, inset=c(-0.09,0.00),cex=2)
...
if(length(args)>=6)
{
	...
	legend(x="topright",legend=c("LOH","HET","ASCNA"),col=c(2:4), pch=20, xpd=NA, inset=c(-0.095,0.00),cex=2)
}
