...
eLOH = LOH.analyze(normal, tumor, strata=the.strata.ls, alpha=0.01,  method="two.sample.fisher")

set.seed(50)
the.loh = multi.LOH.analyze(normal, tumor, all.loh.ls=list(eLOH), test.alpha=0.001, method="two.sample.fisher", sdundo=c(1,2), alpha=c(0.05,0.01))
