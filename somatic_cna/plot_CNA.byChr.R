...
if(length(args)<2)
{
	cat("plot_CNA.byChr.R <infile> <ploidy> [loh file]\n")
	q()
}

dataTable <-read.table(args[1], header=TRUE);
ploidy <- type.convert(args[2])
...
for (i in c(1:22,'X','Y')) 
{
	if (length(args)>2) 
	{
		png(filename=paste(args[1],".chr",i,".png",sep=""),width=800,height=1200,units = "px", pointsize = 20, bg = "white", res = NA)
		op <- par(mfrow=c(2,1))
	}else
	{
		png(filename=paste(args[1],".chr",i,".png",sep=""),width=800,height=600,units = "px", pointsize = 20, bg = "white", res = NA)
	}
	...
	if (length(args)>2) 
	{
		...
		#png(filename = paste(args[6],".chr",i,".png",sep = ""), width = 1180, height = 1180, units = "px", pointsize = 20, bg = "white", res = NA)
