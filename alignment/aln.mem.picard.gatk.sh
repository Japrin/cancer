...
if [ $# -lt 6 ]
then
    lane=`gzip -cd $fq1 | head -n1 | perl -ane '/^@(.+?):(.+?):(.+?):(.+?):/;print "L$4\n"'`
else
    lane=$6
fi
...
if [ ! -f "$outDir/$sampleID.$lib.$lane$optI.sort.bam" ];then
	...
	if [ -f "$outDir/$sampleID.$lib.$lane$optI.sort.bam" ];then
		#echo rm $outDir/$sampleID.$lib.$lane$optI.bam
...
if [ ! -f "$outDir/$sampleID.bam" ];then
    $BinDir/merge_bam.sh $outDir/$sampleID.bam $outDir/$sampleID.$lib.$lane$optI.sort.bam
fi

if [ ! -f "$outDir/$sampleID.markDup.bam" ];then
    $BinDir/rmdup.sh $outDir/$sampleID.bam $outDir/$sampleID.markDup.bam
fi

if [ ! -f "$outDir/$sampleID.markDup.BQSR.bam" ];then
    $BinDir/recal.gatk4.sh $outDir/$sampleID.markDup.bam $outDir/$sampleID.markDup.BQSR.bam
fi
