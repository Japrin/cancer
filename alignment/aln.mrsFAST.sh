...
optM=12
optI=""
_refData="/PROJ/GR/share/medinfo.00database/UCSC/human/chromFaMasked/mrsFAST/hg19.masked.fa"
snpfile="/PROJ/GR/share/medinfo.00database/UCSC/human/chromFaMasked/mrsFAST/dbsnp_137.b37.onlySNP.index"
while getopts c:f:s:t:m:i: opt
do
	case $opt in 
	c)	
		if [ -f $OPTARG ]
	...
	'?')
		...
		echo "Usage: $0 [-c iniFile] [-f refData] [-t threads, default 4] [-m memrory(GB), default 12] [-s snpfile] [-i part] <outDir> <fq1> <fq2, '-' if none> <sampleID> <lib> <lane> "
		exit 1
		;;
	esac
done
...
if [ ! -f "$outDir/$sampleID.$lib.$lane$optI.bam" ];then
	...
	gzip -cd $outDir/$sampleID.$lib.$lane$optI.sam.gz \
		| samtools view -b -S -t $refData.fai - > $outDir/$sampleID.$lib.$lane$optI.tmp.bam 
	java -jar $picardDIR/AddOrReplaceReadGroups.jar \
		...
		CN="novogene"
	if [ -f "$outDir/$sampleID.$lib.$lane$optI.bam" ];then
		rm -f $outDir/$sampleID.$lib.$lane$optI.sam.gz
