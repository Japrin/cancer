...
our ($queryfile, $dbloc);
our ($outfile, $buildver, $remove, $checkfile, $protocol, $operation, $otherinfo, $onetranscript, $nastring, $genericdbfile, $gff3dbfile, $bedfile, $vcfdbfile, $csvout, $argument, $splicing_threshold);
...
GetOptions('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 'outfile=s'=>\$outfile, 'buildver=s'=>\$buildver, 'remove'=>\$remove, 'checkfile!'=>\$checkfile, 
	...
	'csvout'=>\$csvout, 'argument=s'=>\$argument, 'splicing_threshold=i'=>\$splicing_threshold
	) or pod2usage ();
...
sub geneOperation {
	...
	#generate gene anno
	my $sc;
	$sc = "annotate_variation.pl -geneanno -splicing_threshold $splicing_threshold -buildver $buildver -dbtype $protocol -outfile $outfile.$protocol -exonsort $queryfile $dbloc";
...
software to claim variant as regulating splicing. In addition,
if a variant is an exonic variant, it will not be reported as
