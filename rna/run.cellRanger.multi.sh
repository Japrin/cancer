...
templateFile="$shDir/template.cellRanger.multi.config.csv"
...
do
	...
	'?')
	    ...
	    echo "Usage: $0 [-c iniFile] [-a mode. one of 'fastp', and ''. default ''] [-t threads, default 16] [-m memrory(GB), default 128] [-p cellranger parameters] <outDir> <sampleID> <inDir_GEP> <inDir_TCR>"
		exit 1
		;;
	esac
done
