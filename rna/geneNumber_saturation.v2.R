...
if(file.exists(sprintf("%s.RData",out.prefix))){
    ...
    for(tt in c(0,0.1,1,2,3,5,10,50,100)){
        detected.gn[[as.character(tt)]] <- ldply(seq_len(args.m),function(ii){
    ...
    save.image(sprintf("%s.RData",out.prefix))
}
