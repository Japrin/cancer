...
do
	...
	'?')
		...
		echo "Usage: $0 [-t threads, default 8] [-a quality mode, \"--phred33\" or \"--phred64\" (default \"--phred33\")] [-b upto] [-m memrory(GB), default 16] <outDir> <fq1> <fq2, '-' if none> <sampleID> <lib> <lane> "
		exit 1
		;;
	esac
done
...
if [ ! -f "$outDir/$sampleID.hisat.hit.sort.bam" ] && [ ! -f "$outDir/$sampleID.hisat.hit.bam" ];then
	...
	hisat  -x $RRNA_INDEX -1 $outDir/$sampleID.clean.P.R1.fq.gz -2 $outDir/$sampleID.clean.P.R2.fq.gz \
		--minins 0 \
		--maxins 1000 \
		--fr \
		-t \
	...
	### hisat alignment
	hisat  -x $INDEX -1 $outDir/$sampleID.noRRNA.R1.fq.gz -2 $outDir/$sampleID.noRRNA.R2.fq.gz \
