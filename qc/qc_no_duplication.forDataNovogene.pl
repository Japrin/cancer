...
`mkdir -p $outdir/$insertsize` unless(-d "$outdir/$insertsize");
...
if ($end == 2) {
    ...
    #chomp (my $file_1 = `ls $indir/$insertsize/*1_001.fastq.gz`);
    #chomp (my $file_2 = `ls $indir/$insertsize/*2_001.fastq.gz`);
	chomp (my $file_1 = `ls $indir/$insertsize/*_1.fq.gz`);
	chomp (my $file_2 = `ls $indir/$insertsize/*_2.fq.gz`);
	...
	open OUT_1,">:gzip","$outdir/$insertsize/$insertsize.1.clean.fq.gz" or die $!;
	open OUT_2,">:gzip","$outdir/$insertsize/$insertsize.2.clean.fq.gz" or die $!;
...
my $meanQ_errorR = <<FIGURE;
	table<-read.table("$outdir/$insertsize/$insertsize.mean_quality")
        ...
        dev.off()
FIGURE
        open R,"| /usr/bin/env R --vanilla --slave" or die $!;
        print R $GC_figure;
