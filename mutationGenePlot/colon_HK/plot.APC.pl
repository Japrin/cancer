...
=head1 Usage
	...
	perl plot.muc17.pl [option] <outfile> <mutation file> <domain file> <refGene file>
...
sub plotSNVBySeq
{
	...
	while(<$in>)
	{
		...
		$field[3]=~/(.+);/;
		my $aaPos=$1;
		if($aaPos == -1) { $aaPos=aaPos($basePos,\@posCDS,\@rangeCDS); }
		my @tt=split(/;/,$field[6]);
		my $nSample=@tt;
		my $mutType=$field[5];
		##
...
sub plotLegend
{
	...
	$canvas->circle('cx',$x+15,'cy',$y+11,'r',3,'style',{'fill','red'});
	$canvas->text('x',$x+30,'y',$y+14,'font-family','TimesNewRoman','font-size','12','-cdata',"nonsense",'fill','black');
	$canvas->circle('cx',$x+15,'cy',$y+26,'r',3,'style',{'fill','mediumslateblue'});
	$canvas->text('x',$x+30,'y',$y+29,'font-family','TimesNewRoman','font-size','12','-cdata',"missense",'fill','black');
