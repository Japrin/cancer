...
my @funcCata=("exonic","splicing","ncRNA","UTR5","UTR3","intronic","intergenic");
...
while(<$in>)
{
	...
	{
        my $_ffff=$1;
        if($_ffff=~/ncRNA/){ $_ffff="ncRNA"; }
        elsif($_ffff=~/exonic/){ $_ffff="exonic"; }
        elsif($_ffff=~/splicing/){ $_ffff="splicing"; }
        elsif($_ffff=~/UTR5/){ $_ffff="UTR5"; }
        elsif($_ffff=~/UTR3/){ $_ffff="UTR3"; }
        elsif($_ffff=~/intronic/){ $_ffff="intronic"; }
        else{ $_ffff="intergenic"; }
		$func{$_ffff}++;
		$funcTotal++;
		if(/ExonicFunc=(.+?);/)
		{
